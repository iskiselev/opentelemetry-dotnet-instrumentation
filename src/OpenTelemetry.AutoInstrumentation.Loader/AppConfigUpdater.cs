...
#pragma warning restore RS0016
    {
        ...
        {
            throw new ArgumentNullException(nameof(appDomainSetup));
        }

        var patchMode = Environment.GetEnvironmentVariable("OTEL_APP_DOMAIN_STRATEGY");
        if (!Enum.TryParse<PatchMode>(patchMode, ignoreCase: true, out var mode))
        {
            mode = PatchMode.AssemblyRedirect;
        }

        Logger.Information($"Use {mode} strategy for multiple app domains");

        Logger.Information($"ApplicationBase: {appDomainSetup.ApplicationBase}");
        Logger.Information($"PrivateBinPath: {appDomainSetup.PrivateBinPath}");
        Logger.Information($"PrivateBinPathProbe: {appDomainSetup.PrivateBinPathProbe}");
...
#pragma warning restore RS0016
    {
        ...
        try
        {
            Logger.Information($"Try to modify {configPath}");
    ...
    private static void ModifyConfig(XDocument config)
    {
        ...
        foreach (var assemblyInfo in AssemblyCatalog.GetAssemblies().Where(assembly => !assembly.ImplicitRedirect))
        {
            ...
            if (existingRedirects.Count == 1)
            {
                ...
                if (existingNewVersion >= assemblyInfo.Version)
                {
                    ...
                    Logger.Information($"Use existing version. Override version range for {assemblyInfo.FullName.Name}");
